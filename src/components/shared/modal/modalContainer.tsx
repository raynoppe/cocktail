...
export default function Kmodal(props: React.PropsWithChildren<{
    ...
    return (
        <div id={id} className=" fixed z-50 top-0 bottom-0 left-0 right-0 bg-black bg-opacity-40 backdrop-blur-sm flex flex-col items-center justify-center p-5">
            <div className={`bg-white text-gray-700 shadow-[0_35px_40px_15px_rgba(0,0,0,0.5)] rounded-lg animate-slide-top flex flex-col ${useClassBox}`}>
                {props.header && <div className=" bg-primary text-white py-2 px-4 rounded-t-lg flex items-center">
                ...
                {props.footer && <div className={`rounded-b-lg py-2 px-4 border-t border-solid border-gray-200 bg-slate-100 ${useClassFooter}`}>{props.footer}</div>}
            </div>
        </div>
    )
}